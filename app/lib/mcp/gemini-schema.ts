...
function normalizeSchema(
  ...
  const concreteVariants = variantSource.filter((variant) =>
    isObject(variant) && variant.type !== "null");
  if (variantSource.some((variant) => isObject(variant) && variant.type === "null")) {
  ...
  for (const key of ["minimum", "maximum"]) {
    ...
    if (typeof schema[key] === "number" && Number.isFinite(schema[key])) {
      ...
      diagnostics.warnings.push(`${path}.${key} was ignored because it must be a finite number.`);
    }
  }
