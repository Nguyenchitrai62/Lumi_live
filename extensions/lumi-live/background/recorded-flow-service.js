...
function preserveLocalUploadBindings(importedFlow, existingFlow) {
  ...
  const usedExistingUploads = new Set();
  for (const importedUpload of uploadSteps(imported)) {
    const expectedCount = Math.max(
      1,
      importedUpload.fileVariables?.length || importedUpload.files?.length || 1,
    );
    const existingIndex = existingUploads.findIndex((candidate, index) => (
      !usedExistingUploads.has(index)
      && uploadBindingKey(candidate) === uploadBindingKey(importedUpload)
      && Array.isArray(candidate.localFilePaths)
      && candidate.localFilePaths.length === expectedCount
    ));
