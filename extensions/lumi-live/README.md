...
The built-in `get_transcript(url)` and `video_summary(url)` tools classify each required URL as YouTube, Facebook, or Udemy inside the tool. Both routes try a complete exact caption/subtitle track first and prefer English when several complete tracks exist. YouTube then goes directly to Gemini Flash-Lite by public watch URL. Facebook and Udemy resolve verified audio and use optional Groq Whisper only as a speed optimization: a successful Groq transcript is used immediately, while a missing or failed Groq call falls back to Gemini. Summary fallback sends the verified audio to Gemini once rather than generating a separate intermediate Gemini transcript. Transcript requests add a downloadable `.txt` card for the current request; the transcript payload is excluded from reusable chat/task history and is not cached as a prior video analysis.
...
The chat box remains editable while Lumi is responding or reconnecting. Sending during an active response adds the message to a visible queue and sends it automatically when the current turn finishes. Choose **Steer** on the queue row to interrupt the current turn and send that message immediately, or use the trash icon to remove it. Final user and Lumi text turns are organized into local chat sessions in `chrome.storage.local` and restored when the side panel opens again. **New chat always performs a hard context reset**, even when the visible chat is already empty: pending browser/MCP/video work is aborted, queued and partial state is cleared, and old prompt targets, tool state, and local conversation context are discarded before the next request. New chat never closes, replaces, resumes, or reopens the Gemini WebSocket; that transport opens with the side panel and closes with the side panel. Every new prompt captures its active tab again and carries the prompt-locked title and URL, so “current video” cannot reuse an earlier chat's page. Late output from a genuinely cancelled turn is ignored. If any empty New chat already exists, Lumi can reuse that local session while still performing the same hard context/task reset. The conversation selector opens a desktop-style history panel for switching between or deleting older sessions, and delete/Clear all confirmations remain inside the side panel. A legacy single transcript is migrated into the first named session automatically. Lumi retains up to 50 sessions, 200 turns and 250,000 characters per session, with a 1,500,000-character total context cap. When a fresh socket has to rebuild conversational memory, it supplies at most the latest 10 turns and 18,000 characters: the newest user request is retained whole within its generous per-turn allowance, while older user/Lumi turns preserve their beginning and conclusion with low-priority middle detail omitted locally. This deterministic projection does not call a model or add a summarization turn. The current task is handled separately: its request anchor, remaining-goal ledger, durable identifiers, complete newest tool observation, browser controls, and verification evidence are never thinned by the chat-history policy. Fast full-page state now has a substantially larger allowance for large forms. In parallel, each session keeps a complete local IndexedDB conversation snapshot containing user/Lumi messages, Thinking cards, every Lumi Task step and result, MCP activity, captured screenshots, attached-image previews, disclosure state, and scroll position. On-demand video transcript payloads and their transient download cards are excluded. Restored task cards remain expandable and their structured event history continues with collision-free IDs. **Clear all** removes both context metadata and complete conversation snapshots.

The chat composer accepts up to five attachments per message: XLSX/CSV spreadsheets and at most one JPEG, PNG, WebP, or GIF image. Files can be dropped anywhere in the side panel; the panel-wide handler passes every drop through the shared attachment classifier so future supported file types inherit the same behavior. Lumi parses spreadsheets locally in a Web Worker and registers each file under a session-only `workbookId`. Gemini receives a read-only `excel_understand` tool with `overview`, `read_range`, `search`, and `formulas` modes; the dedicated formula mode lists formula cells without scanning every populated range. It preserves sheet names, sparse cell addresses, formulas, cached results, number formats, comments, hyperlinks, merges, hidden-sheet state, and defined names; every read call is capped at 2,000 populated cells and returns a continuation cursor when more data remains. `read_range` accepts either `A3:A40` or a matching qualified reference such as `'Tóm tắt'!A3:A40`, but rejects a qualifier that conflicts with the separate `sheet` argument. XLSX attachments also expose an always-enabled `excel_edit` tool for bounded `set_formula`, `set_value`, and `clear` operations and for exporting the current session revision as a downloadable `.xlsx` copy. Edits patch worksheet XML in a copied archive, preserve unrelated workbook parts, request formula recalculation on open, reject complex formula ranges or non-anchor merged cells that cannot be changed safely, and never overwrite the user's source file. CSV editing is intentionally unsupported. The initial prompt contains spreadsheet metadata only, while exact bounded cell/search/formula results are returned to Gemini only when needed for the user's request. Workbook content is never written to chat history, `chrome.storage`, or IndexedDB, and its IDs and temporary export URLs expire on New chat, chat switch, reload, or panel close. Images retain the existing local resize and Gemini Live video-frame behavior.
...
1. Include a full YouTube, Facebook video/Reel, or Udemy lecture link in the request.
2. A transcript request calls `get_transcript` with `{ url }`; a summary request calls `video_summary` with `{ url, outputLanguage? }`. There is no public `action`, `sourceUrl`, audio-link, `both`, or cached `inspect` mode.
3. Each tool classifies `url` internally and first tries a complete caption belonging to the exact video, preferring English when several complete tracks exist: live YouTube timed text, Facebook caption metadata tied to the exact Reel ID, or Udemy tracks, embedded player data, and authenticated in-frame caption requests. Every explicitly supplied Facebook URL is loaded in a fresh temporary background tab—even when it matches the current Reel—so a long-lived scrolling feed cannot leak adjacent or stale media into discovery. In Fast mode that tab joins **Agent Space** before inspection and is always closed after success, failure, cancellation, or timeout.
4. Without a usable caption, YouTube goes directly to Gemini 3.5 Flash-Lite using the public watch URL, with Gemini 3.1 Flash-Lite as the capacity/quota fallback. It does not extract YouTube audio for Groq.
5. Without a usable Facebook/Udemy caption, Lumi resolves verified audio. If a Groq key exists, it tries `whisper-large-v3` first for higher transcription quality and falls back to `whisper-large-v3-turbo` when the first model is rate-limited or temporarily unavailable. Groq is optional: missing keys, limits, authentication errors, and other failures fall back to Gemini. Transcript requests ask Gemini to transcribe the audio; summary requests ask Gemini to summarize the audio directly in one pass.
6. `get_transcript` shows a **Download transcript** card for the current request. Its full payload is removed before the tool result enters Gemini Live/task history and is omitted from local conversation snapshots, so it cannot be reused by a later chat.
...
| `documents/` | Local XLSX/CSV parsing, formula inspection, safe XLSX editing, and session-only Excel tool registry |
...
`manifest.json` loads the bundled `dist/background.js`, `side-panel/index.html`, and `settings/index.html`. `extensions/build.mjs` bundles `background/index.js` (including MP4Box for AAC extraction), `browser/controller.js`, and the isolated `documents/document-parser-worker.js` into `dist/`. `npm test` validates these paths and recursively resolves every local JavaScript import reachable at runtime.
...
The Gemini key, optional Groq key, selected voice, thinking level, Fast mode state, avatar preference, chat-session context metadata, MCP servers, dynamically registered OAuth client metadata, OAuth tokens, Redmine API keys, and tool policies are stored in `chrome.storage.local`, which is isolated to the extension's Chrome profile and is not synchronized through `chrome.storage.sync`. Visual conversation snapshots are stored per session in the extension's local IndexedDB database; transient video transcripts and attached spreadsheet contents are excluded. Removing a session deletes its IndexedDB snapshot, and **Clear all** clears the snapshot store. Removing a connector deletes its saved connector identity and tokens. Connector authorization codes, tokens, OAuth credentials, and API keys are never sent to a Lumi-owned backend; they are sent only to the relevant provider or authorized MCP/Redmine endpoint. When the user attaches and asks Lumi to analyze or edit a spreadsheet, the metadata, exact bounded cell/search/formula results, and requested edit arguments needed for that task are sent to the user's configured Gemini service; the full workbook archive remains inside the side panel and is not automatically inserted into the prompt. For a user-requested Facebook/Udemy transcript or summary, verified direct audio URLs or small audio files prepared inside the extension can be sent to Groq when its key is configured. No Lumi backend or loopback helper is used for video analysis.
