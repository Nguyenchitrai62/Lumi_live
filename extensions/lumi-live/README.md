...
## Normal, Fast, and Background workspace

Normal and Fast are execution policies over one shared Gemini Live agent. They use the same full-page PageAgent semantic index, Page Map, `stateId`, document identity, page delta, task memory, safety checks, verification, retry policy, and completion contract. Switching modes does not reconnect Gemini or replace the page controller.

- **Normal execution** prioritizes presentation. It keeps the avatar and panel motion, animates explicit scrolling and text entry, and shows a compact stage progress card. Small stages animate every edit; large stages animate representative edits while the remaining controls execute directly, so visual polish does not add model round trips.
- **Fast execution** prioritizes throughput. It removes decorative effects and executes the same verified plan in zero-delay chunks. It is intended for complex stages that may fill or tick hundreds of independent controls, without weakening context, reasoning, safety, or verification.
- **Background workspace** is an independent target policy available in Settings. It creates a purple **Lumi Workspace** tab group whose target can remain controllable while the user views another tab. It can be combined with either Normal or Fast execution. When disabled, either execution mode follows the active tab.

`browser_apply_stage` is the preferred bulk primitive in both modes. It binds up to 300 resolved edits to the latest `stateId`, and can expand named checkbox/radio scopes locally so Gemini does not need to emit hundreds of indices. Before changing anything, the controller resolves the complete plan, rejects ambiguous scopes, duplicates, unsupported controls, conflicting radios, secrets, and invalid options. It skips already-correct selection state, executes bounded chunks, verifies every applied value, verifies the final stage, and keeps a local paged ledger. A page change or replaced control stops the stage with a compact checkpoint; Lumi observes fresh state and rebuilds only the remaining edits. Navigation, submission, saving, publishing, deletion, and controls that reveal later prerequisites remain separate barriers.

The shared context engine indexes rendered controls across the complete DOM, including off-viewport controls, accessible label and description relationships, open Shadow DOM, and same-origin frames. It returns a compact Page Map and only the changed controls between observations. Scrolling is reserved for explicit user requests or genuinely virtualized content that is not yet in the DOM.
...
## Browser automation design provenance

The execution core adapts the strongest relevant patterns from [browser-use](https://github.com/browser-use/browser-use): DOM-first observation, state-bound indexed actions, complete multi-action preflight, page-change and disconnected-control guards, partial results, compact state history, and unchanged-state loop detection. Lumi-specific optimizations remain local to this extension: one Gemini Live planner for both modes, semantic scope expansion inside the page, one StagePlan with two presentation policies, representative Normal animations, zero-delay Fast chunks, a paged in-browser ledger, and a target workspace that is independent of execution mode.
...
- **PAGEAGENT TARGET** is controlled by the independent target policy. With Background workspace disabled, Normal and Fast both follow the active HTTP, HTTPS, or permitted `file://` tab. With it enabled, either mode uses the dedicated purple **Lumi Workspace** group.
- Page element guides are optional and disabled by default.
- Tab listing and switching follow the target policy rather than execution mode. Active-tab policy exposes the current window and visibly activates destinations; Background workspace exposes only group members and keeps agent-opened destinations inactive in the group. Normal animates explicit scrolling and input; Fast performs them immediately. A `text` target reveals matching rendered content, while `position=0` is the exact top, `position=0.5` the middle, and `position=1` the exact bottom.
