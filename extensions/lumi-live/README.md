...
5. Without a usable Facebook/Udemy caption, Lumi resolves verified audio. If a Groq key exists, it tries `whisper-large-v3-turbo` and then `whisper-large-v3`. Groq is optional: missing keys, limits, authentication errors, and other failures fall back to Gemini. Transcript requests ask Gemini to transcribe the audio; summary requests ask Gemini to summarize the audio directly in one pass.
