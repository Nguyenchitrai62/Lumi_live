...
The chat box remains editable while Lumi is responding or reconnecting. Sending during an active response adds the message to a visible queue and sends it automatically when the current turn finishes. Choose **Steer** on the queue row to interrupt the current turn and send that message immediately, or use the trash icon to remove it. Final user and Lumi text turns are stored in `chrome.storage.local` on the user's device and restored when the side panel opens again. The local archive retains up to 200 recent turns and 250,000 characters; only the latest 32 turns and 24,000 characters are supplied back to Gemini so reconnect latency and model context remain bounded. Thinking text, tool traces, task-step details, screenshots, and attached image data are not persisted. The topbar trash button clears the saved history and starts a new conversation.
