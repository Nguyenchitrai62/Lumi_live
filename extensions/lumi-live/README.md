...
Use the **Fast** button in the side-panel topbar, or the **Fast mode** switch in Settings, when throughput matters more than presentation. Fast mode creates a yellow **⚡ Lumi Fast** Chrome tab group and pins PageAgent's target to that workspace. The agent can keep reading and changing its inactive workspace tab while the user works in another tab; agent-opened destinations join the group without taking focus. PageAgent indexes every rendered interactive element across the complete DOM in one observation, including off-viewport controls, and semantic queries resolve accessible labels, descriptions, open Shadow DOM, and same-origin frames. Those indices can be clicked, filled, selected, or batched directly without exploratory viewport scrolling. Explicit vertical or horizontal scrolling remains available and executes as an immediate jump with no animation; genuinely virtualized content still has to enter the DOM before it can be indexed. Fast mode also hides both avatar renderers and the petal/background scene, stops their animation loops, removes panel transitions and transcript reveal effects, disables PageAgent highlights, pointer/mask effects, gradual typing, and the Google departure animation. It enables `browser_batch_actions`, which applies up to 100 independent checkbox, radio, switch, text-field, and HTML-select edits from one fresh full-page indexed state. Already-correct selection controls can be skipped with an explicit desired state. Navigation and consequential final actions remain separate so the normal verification and confirmation boundaries still apply. Visual screenshots still require the workspace tab to be visible; background work uses PageAgent's semantic DOM state. Turning Fast mode off ungroups (but does not close) its tabs and restores the original active-tab-following behavior.
