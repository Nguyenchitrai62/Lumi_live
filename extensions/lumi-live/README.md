...
## Normal and Fast execution

Normal and Fast run through one shared Gemini Live agent. They use the same full-page PageAgent semantic index, Page Map, `stateId`, document identity, page delta, task memory, safety checks, verification, retry policy, and completion contract. Switching modes does not reconnect Gemini or replace the page controller.

- **Normal execution** follows the active Chrome tab and prioritizes presentation. It keeps the avatar and panel motion, animates explicit scrolling and text entry, and shows a compact stage progress card. Small stages animate every edit; large stages animate representative edits while the remaining controls execute directly, so visual polish does not add model round trips.
- **Fast execution** creates or restores the yellow **⚡ Lumi Fast** tab group and is strictly limited to its members. The selected group tab remains Lumi's target when the user moves to another tab, so a task can continue in the background. Tabs outside the group are neither listed nor controllable, and `browser_switch_tab` cannot import them. Agent-opened destinations join the group without taking focus. Fast also removes decorative effects and executes the same verified plan in zero-delay chunks, making it suitable for stages that fill or tick hundreds of independent controls.

Turning Fast off ungroups—but never closes—its workspace tabs and restores Normal active-tab following.
...
- **PAGEAGENT TARGET** follows the active HTTP, HTTPS, or permitted `file://` tab in Normal mode. Fast mode pins it to the dedicated yellow **⚡ Lumi Fast** group and keeps that target controllable in the background.
- Page element guides are optional and disabled by default.
- Normal tab listing exposes the current window and visibly activates destinations. Fast tab listing exposes only **⚡ Lumi Fast** members, rejects outside tab IDs, and keeps agent-opened destinations inactive inside the group. Normal animates explicit scrolling and input; Fast performs them immediately. A `text` target reveals matching rendered content, while `position=0` is the exact top, `position=0.5` the middle, and `position=1` the exact bottom.
