...
test("extension pre-authorizes the exact action tab without a share picker", async () => {
  ...
  const offscreen = await readFile(new URL("./offscreen-translate.js", import.meta.url), "utf8");
...
test("extension automatically falls back to the active HTML media element", async () => {
  const worker = await readFile(new URL("./service-worker.js", import.meta.url), "utf8");
  const pageController = await readFile(new URL("./page-controller.js", import.meta.url), "utf8");
  const offscreen = await readFile(new URL("./offscreen-translate.js", import.meta.url), "utf8");
...
test("live translation uses jitter-buffered low-latency streaming without changing playback speed", async () => {
  const controller = await readFile(new URL("./live-translate.js", import.meta.url), "utf8");
  const worklet = await readFile(new URL("./pcm-capture-worklet.js", import.meta.url), "utf8");
  const offscreen = await readFile(new URL("./offscreen-translate.js", import.meta.url), "utf8");
