...
test("releases Fast workspace only after a confirmed panel close", async () => {
  ...
  assert.match(worker, /createSidePanelLifecycle/);
  assert.match(worker, /chrome\.sidePanel\.onClosed\.addListener/);
  assert.match(worker, /fastWorkspace\.release\(\{ shouldRelease: isCurrent \}\)/);
  assert.match(worker, /activateWorkspace = sidePanelLifecycle\.isOpen/);
  assert.doesNotMatch(worker, /sidePanelPorts\.size === 0\) await fastWorkspace\.release/);
  assert.match(panelController, /createResilientRuntimePort/);
