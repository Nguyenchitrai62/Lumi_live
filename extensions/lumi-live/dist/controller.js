  ...
  var dom_tree_default = (args = {
    ...
    */
    ...
    function isHeuristicallyInteractive(element) {
      if (!element || element.nodeType !== Node.ELEMENT_NODE) return false;
      if (!isElementVisible2(element)) return false;
      ...
      const hasVisibleChildren = [...element.children].some(isElementVisible2);
    ...
    function buildDomTree(node, parentIframe = null, isParentHighlighted = false) {
      ...
      if (node.nodeType === Node.ELEMENT_NODE) {
        nodeData.isVisible = isElementVisible2(node);
...
${clippedAnchor.content}`);
  ...
  function comparableText(value) {
    return compactText2(value, 1e3).normalize("NFKC").toLocaleLowerCase();
  }
  function attributeSelectorValue2(value) {
    return String(value).replace(/\\/g, "\\\\").replace(/"/g, '\\"');
  }
  ...
  function accessibleName2(element) {
    ...
    return compactText2(
      element?.getAttribute?.("aria-label") || labelledText || associatedLabel2(element) || element?.getAttribute?.("title") || element?.getAttribute?.("alt") || element?.getAttribute?.("placeholder") || element?.innerText || element?.textContent || (["button", "submit", "reset"].includes(String(element?.type || "").toLowerCase()) ? element?.value : "")
    );
  }
  function elementDescriptor2(element) {
    ...
    return {
      ...
      role: compactText2(element?.getAttribute?.("role"), 80).toLowerCase(),
  ...
  function scoreText(actual, expected, exactScore, partialScore = Math.floor(exactScore / 2)) {
    ...
    if (normalizedActual.length >= 3 && (normalizedActual.includes(normalizedExpected) || normalizedExpected.includes(normalizedActual))) return partialScore;
    return 0;
  }
  ...
  function scoreRecordedTargetCandidate(target, element) {
    ...
    if (target.elementId && descriptor.elementId === target.elementId && (!isLikelyDynamicElementId2(target.elementId) || !hasSemanticIdentity(target) || semanticScore >= 35)) score += 130;
  ...
  function rankRecordedTargetCandidates(rawTarget, {
    ...
    if (target.testId) {
      addCandidates(
        candidates,
        queryRoots(`[data-testid="${attributeSelectorValue2(target.testId)}"]`),
        "testId",
        500,
        target
      );
    ...
    if (target.elementId) {
      addCandidates(
        candidates,
        queryRoots(`[id="${attributeSelectorValue2(target.elementId)}"]`),
        "elementId",
        450,
        target
      );
    ...
    if (target.inputName) {
      ...
      addCandidates(
        candidates,
        queryRoots(`${tag}[name="${attributeSelectorValue2(target.inputName)}"]`),
        "inputName",
        350,
        target
      );
  ...
  function isElementEnabled(element, windowObject) {
    ...
    for (let current = element; current; current = current.parentElement) {
      ...
      if (current.disabled === true || current.inert === true || current.matches?.(":disabled") === true || current.hasAttribute?.("disabled") === true || current.getAttribute?.("aria-disabled") === "true" || disabledStateValue(current, "data-disabled") || current.getAttribute?.("data-state") === "disabled" || classList?.contains?.("disabled") || classList?.contains?.("is-disabled") || style?.pointerEvents === "none") return false;
    }
    return true;
  }
  function rectSignature(element) {
    const rect = element.getBoundingClientRect?.();
    if (!rect) return "";
    return [rect.x, rect.y, rect.width, rect.height].map((value) => Math.round(Number(value) * 10) / 10).join(":");
  }
  ...
  function fillElement(element, value, windowObject) {
    ...
    const actualValue = element.isContentEditable ? String(element.textContent ?? "") : String(element.value ?? "");
  ...
  var GLOBAL_KEY = `__LUMI_PAGE_AGENT_CONTROLLER_V${PAGE_CONTROLLER_PROTOCOL_VERSION}__`;
