...
export const BROWSER_TOOLS = [
  ...
  },
  {
    ...
    description: "List all tabs available under the current target policy and report whether each supports PageAgent control. Execution mode does not change tab scope. When Background workspace is enabled, only its group members are listed; otherwise all tabs in the current Chrome window are listed. Always call this immediately before browser_switch_tab.",
    parameters: { type: "OBJECT", properties: {} },
  },
  {
    name: "browser_open_tab",
    description: "Open an absolute http, https, or file URL only when no matching tab already exists. File URLs require the user to enable Chrome's Allow access to file URLs setting for Lumi. From New Tab, chrome:// pages, and other restricted pages under the active-tab target policy, Lumi must first open exactly https://www.google.com/, then reuse that same tab for the destination. Target policy, not Normal/Fast execution mode, decides whether the destination becomes active or opens inactive inside Lumi's background workspace.",
    parameters: {
      type: "OBJECT",
      properties: {
  ...
  },
  {
    ...
    description: "Activate any existing Chrome tab returned by the latest browser_list_tabs result as the agent target. Target policy, not Normal/Fast execution mode, decides whether it becomes visibly active or remains in the background workspace.",
    parameters: {
      type: "OBJECT",
      properties: {
...
Normal and Fast are execution policies only. They never change the semantic context, reasoning quality, task memory, completion contract, safety rules, or target scope. The independent Background workspace target policy decides whether Lumi follows the active http, https, or file tab (when Chrome permits file access) or locks prompts to a named Chrome tab group whose tabs remain controllable in the background. When Background workspace is active, browser_list_tabs exposes only workspace members, browser_switch_tab rejects outside tabs, and browser_open_tab may create a necessary inactive workspace tab. Otherwise Lumi follows the active Chrome tab and tab navigation is visible. A file tab supports PageAgent only after the user enables Chrome's Allow access to file URLs setting for Lumi. The navigation tools browser_list_tabs, browser_open_tab, and browser_switch_tab remain available from every active tab, including restricted pages; browser_open_tab accepts absolute http, https, and file URLs. Before opening or switching tabs, call browser_list_tabs, reuse a matching listed tab, and open a new tab only when no match exists. Under the active-tab target policy, from a restricted active page browser_open_tab must open exactly https://www.google.com/ in a new active tab, reuse that same tab for the destination without leaving a spare Google tab, and never substitute another Google domain, search URL, or website. Never ask the user to switch away from an uncontrollable page when navigation tools can advance the request. When a request includes opening or starting a YouTube video, perform the relevant browser_click without a spoken preamble. After navigation, a tab switch, or a page-changing click, obtain a fresh shared page state before an indexed action.
...
function formatFastModeSessionContext(fastMode) {
  return fastMode
    ? "Fast execution is enabled at session start. It uses the same shared full-page context, reasoning, StagePlan, safety, verification, and memory as Normal mode, but disables decorative effects and runs browser_apply_stage in zero-delay chunks. Target scope is controlled independently by the Background workspace setting. A later page-state result is authoritative if execution mode changes."
    : "Normal execution is enabled at session start. It uses the same shared full-page context, reasoning, StagePlan, safety, verification, and memory as Fast mode, while presenting representative page animations and visible stage progress. Large independent edits should still use browser_apply_stage so presentation never adds Gemini round trips. A later page-state result is authoritative if execution mode changes.";
}
