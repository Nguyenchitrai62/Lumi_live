...
export const BROWSER_TOOLS = [
  ...
  },
  {
    ...
    description: "List tabs available to the current mode and report whether each supports PageAgent control. In Normal mode this lists the current Chrome window. During a Fast-mode user prompt it returns only the single locked Agent Space tab; every other workspace tab is intentionally excluded from agent context. Do not use it to change which existing tab a Fast-mode prompt targets.",
    parameters: { type: "OBJECT", properties: {} },
  },
  {
  ...
  },
  {
    ...
    description: "Activate an existing Chrome tab returned by browser_list_tabs. In Normal mode the tab becomes visibly active. A Fast-mode prompt is locked to one Agent Space tab, so switching to any other existing workspace tab is rejected until the next user prompt.",
    parameters: {
      type: "OBJECT",
      properties: {
...
function formatFastModeSessionContext(fastMode) {
  return fastMode
    ? "Fast mode is enabled at session start. It keeps the same Gemini Live planning, remaining-goal ledger, durable memory, recovery, safety, and verification harness as Normal mode. Decorative browser and side-panel effects are disabled. Each user prompt is locked to exactly one tab: the user's active controllable tab is added to Agent Space when necessary, and every other Agent Space tab is omitted from that prompt's context. Do not inspect, compare, summarize, transcribe, or switch to those other tabs. The locked target remains authoritative in the background until the next user prompt. PageAgent indexes the complete rendered DOM and makes off-viewport controls directly actionable, so use browser_find_semantic_context and never scroll merely to discover content or obtain indices. Prefer browser_set_selection for many controls sharing one desired state and browser_batch_actions for mixed independent form edits after one fresh full-page indexed observation. Explicit horizontal or vertical scrolls execute instantly. Use semantic DOM inspection for the locked background tab; request a screenshot only when it is visibly active."
